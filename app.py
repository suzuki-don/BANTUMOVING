...
def E_AEs(Pc):
    Z = bytes.fromhex(Pc)
    key_local = bytes([89, 103, 38, 116, 99, 37, 68, 69, 117, 104, 54, 37, 90, 99, 94, 56])
    iv = bytes([54, 111, 121, 90, 68, 114, 50, 50, 69, 51, 121, 99, 104, 106, 77, 37])
    K = AES.new(key_local , AES.MODE_CBC , iv)
    R = K.encrypt(pad(Z , AES.block_size))
    return bytes.fromhex(R.hex())

def generate_random_name():
    characters = string.ascii_letters + string.digits
    random_part = ''.join(random.choice(characters) for _ in range(6)).upper()
...
def token(uid , password , region):
    ...
    try:
        jresp = response.json()
        open_id = jresp.get('open_id')
        access_token = jresp.get("access_token")
        refresh_token = jresp.get('refresh_token')
        ...
        return {"ok": False, "error": "token_exception", "detail": str(e)}
...
def Major_Regsiter(access_token , open_id , field , uid , password, region):
    ...
    headers = {
        ...
        "Content-Type": "application/x-www-form-urlencoded",
        "Expect": "100-continue",
        "Host": "servers.prv.ob52.astutech.online",
        "ReleaseVersion": "OB52",
...
def encrypt_api(plain_text):
    ...
    key_local = bytes([89, 103, 38, 116, 99, 37, 68, 69, 117, 104, 54, 37, 90, 99, 94, 56])
    iv = bytes([54, 111, 121, 90, 68, 114, 50, 50, 69, 51, 121, 99, 104, 106, 77, 37])
    cipher = AES.new(key_local, AES.MODE_CBC, iv)
...
def login(uid , password, access_token , open_id, response , status_code , name , region):
    ...
    if RESPONSE.status_code == 200:
        ...
        if lang.lower() not in ["ar", "en"]:
            ...
            BASE64_TOKEN = RESPONSE.text[RESPONSE.text.find("eyJhbGci"):-1]
        ...
        return dat
    return {"ok": False, "error": "major_login_http", "status_code": RESPONSE.status_code}
...
def login_server(uid , password, access_token , open_id, response , status_code , name , region):
    ...
    if RESPONSE.status_code == 200:
        ...
        dat = GET_PAYLOAD_BY_DATA(BASE64_TOKEN,access_token,1,response , status_code , name , uid , password, region)
        return dat
    return {"ok": False, "error": "login_server_http", "status_code": RESPONSE.status_code}
...
def GET_PAYLOAD_BY_DATA(JWT_TOKEN, NEW_ACCESS_TOKEN, date, response, status_code, name, uid, password, region):
    try:
        ...
        # EXTRACT ACCOUNT ID
        account_id = decoded_json.get("account_id")
        
        NEW_EXTERNAL_ID = decoded_json['external_id']
        SIGNATURE_MD5 = decoded_json['signature_md5']
        ...
        print("Error in GET_PAYLOAD_BY_DATA:", e)
        return None
...
def GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region):
    ...
    while attempt < max_retries:
        try:
            ...
            print("Request failed:", e, "Attempt", attempt + 1, "of", max_retries, "Retrying...")
            attempt += 1
            time.sleep(2)
...
def create_route():
    global CUSTOM_NAME, CUSTOM_PASS
    region = request.args.get("region")
    if not region:
        return jsonify({"status": "error", "detail": "region parameter required. Example: /create?region=BD"}), 400

    custom_name = request.args.get("name")
    custom_pass = request.args.get("password")

    CUSTOM_NAME = custom_name
    CUSTOM_PASS = custom_pass

    try:
        result = create_acc(region.upper())
        
        CUSTOM_NAME = None
        CUSTOM_PASS = None

        if isinstance(result, dict) and result.get("uid") and result.get("password"):
            ...
            final_response = {
                ...
                "tokens": {
                    ...
                    "signature_md5": result.get("signature_md5")
                }
            }
        ...
        return jsonify({
            ...
            "raw_response": str(result) if result else "No response"
        }), 500
    except Exception as e:
        CUSTOM_NAME = None
        CUSTOM_PASS = None
...
def health_check():
    return jsonify({
        "status": "healthy",
        "service": "FreeFire Account Creator",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    })
...
def list_regions():
    return jsonify({
        ...
        "region_languages": REGION_LANG,
        "region_urls": REGION_URLS
    })

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.getenv("PORT", 20377)), debug=False)
